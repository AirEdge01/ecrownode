...
const sendAdminOrderAlert = async (orderData) => {
  ...
  try {
    ...
    console.log("✉️ Admin order alert email sent.");
  } catch (err) {
...
const sendWelcomeEmail = async (userEmail, userName = 'Valued Customer') => {
  ...
  const mailOptions = {
    ...
    html: `
      <div style="font-family: Arial, sans-serif; padding: 20px; color: #333; line-height: 1.6;">
        ...
        <p>You can now browse items, save products to your cart, and place orders smoothly.</p>
  ...
  try {
    await transporter.sendMail(mailOptions);
    console.log(`✉️ Welcome email dispatched to ${userEmail}`);
  } catch (err) {
    console.error("❌ Failed to send Welcome Email:", err.message);
  }
};

// Signin / Login Security Alert Email
const sendLoginAlertEmail = async (userEmail, userAgent = 'Unknown Device') => {
  ...
  const mailOptions = {
    ...
    to: userEmail,
    subject: `🔐 Security Alert: Successful Sign-In to eCrown`,
    html: `
      <div style="font-family: Arial, sans-serif; padding: 20px; color: #333; line-height: 1.6;">
        <h2>New Account Sign-In Detected</h2>
        <p>We recorded a successful sign-in to your eCrown account.</p>
        <ul>
          <li><strong>Time:</strong> ${timeStamp}</li>
          <li><strong>Device Info:</strong> ${userAgent}</li>
        </ul>
        <p style="color: #555;">If this was you, no further action is required.</p>
        <p style="color: #d9534f; font-weight: bold;">If you did not authorize this login, please reset your password immediately.</p>
      </div>
    `
  };
  ...
  try {
    await transporter.sendMail(mailOptions);
    console.log(`✉️ Login notification email dispatched to ${userEmail}`);
  } catch (err) {
