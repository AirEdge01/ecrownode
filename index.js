...
app.use(express.json());
app.use(cors({
  origin: ['https://e-crown-8duf.vercel.app', 'https://localhost:5174'],
  methods: ['GET', 'POST', 'PUT', 'DELETE', 'PATCH'],
  credentials: true
}));

// ── ROUTES ────────────────────────────────────────────────────────────────────
app.get('/', (req, res) => res.send('eCrown API running.'));
...
app.post('/api/orders', async (req, res) => {
  try {
    ...
    if (!incomingItems || !Array.isArray(incomingItems) || incomingItems.length === 0) {
      return res.status(400).json({ message: "Checkout items cannot be empty." });
    }
    ...
    for (const item of incomingItems) {
      ...
      if (product) {
        if (product.stock <= 0) {
          return res.status(400).json({ success: false, outOfStock: true, productId: product._id, message: `"${product.name}" is OUT OF STOCK!` });
        }
        if (product.stock < item.quantity) {
        ...
        formattedItems.push({ productId: String(targetId), name: item.name || 'Unknown Item', quantity: Number(item.quantity || 1), price: Number(item.price || 0) });
      }
    }

    const savedOrder = await new Order({ email: customerEmail, userEmail: customerEmail, items: formattedItems, totalAmount: Number(totalAmount), reference, status: reference ? 'Paid' : 'Pending' }).save();

    for (const item of formattedItems) {
      if (mongoose.Types.ObjectId.isValid(item.productId)) {
        const p = await Product.findById(item.productId);
        if (p) await Product.findByIdAndUpdate(item.productId, { $inc: { stock: -item.quantity } });
      }
    }

    sendAdminOrderAlert(savedOrder).catch(err => console.error("Mail error:", err));

    return res.status(201).json({ success: true, message: "Order saved successfully!", order: savedOrder, orderId: savedOrder._id });

  } catch (error) {
    console.error("❌ ORDER ERROR:", error);
    return res.status(500).json({ message: "Server error.", databaseError: error.message });
  }
});
...
app.get('/api/admin/orders', async (req, res) => {
  try {
    ...
    const normalizedOrders = orders.map(o => ({
      _id: o._id, userEmail: o.userEmail || o.email, totalAmount: o.totalAmount,
      reference: o.reference || "N/A", status: o.status, createdAt: o.createdAt
    }));
    return res.json({ success: true, orders: normalizedOrders });
  } catch (error) { return res.status(500).json({ error: error.message }); }
});
